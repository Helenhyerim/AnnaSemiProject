...
public class FaqEditFormController extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	super.setRedirect(false);
			...
			if(!"POST".equalsIgnoreCase(method)) { // GET 이라면
				...
				super.setRedirect(false);
				super.setViewPage("/WEB-INF/view/faqCBJ/faqEditForm.jsp");
			...
			// ==== 파일을 업로드 해준다. 시작 ==== //
				try {
	                ...
	                request.setAttribute("loc", request.getContextPath()+"/faqEditForm.an"); 
			...
			// === 첨부 이미지 파일을 올렸으니 그 다음으로 제품정보를 (제품명, 정가, 제품수량,...) DB의 tbl_product 테이블에 update 를 해주어야 한다.  ===
				...
				String faqNo = mtrequest.getParameter("faqNo");
				
				InterFaqDAO edao = new FaqDAO();
				
				FaqVO evo = new FaqVO();
				evo.setFaqNo(Integer.parseInt(faqNo));
				evo.setFaqTitle(faqTitle);
				evo.setFaqImg(faqImg);
				...
				try {
					...
					message = "수정 실패!!";
					loc = request.getContextPath()+"/faqEditForm.an";
				}
