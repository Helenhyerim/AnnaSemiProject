...
public class QuestionEditController extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!"POST".equalsIgnoreCase(method)) { // GET 이라면
			...
			super.setViewPage("/WEB-INF/view/qnaCBJ/questionEdit.jsp");
		...
		// ==== 파일을 업로드 해준다. 시작 ==== //
			try {
                ...
                request.setAttribute("loc", request.getContextPath()+"/questionEdit.an"); 
		...
		// === 첨부 이미지 파일을 올렸으니 그 다음으로 제품정보를 (제품명, 정가, 제품수량,...) DB의 tbl_product 테이블에 insert 를 해주어야 한다.  ===
			...
			try {
				...
				message = "수정 실패!!";
				loc = request.getContextPath()+"/questionEdit.an";
			}
